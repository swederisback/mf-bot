...
const giveawayCommand = [
    ...
    {
        name: "ga-forcestart-locked",
        description: "[ADMIN] Force start a giveaway",
        type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND
    },
    {
        name: "ga-forcestop-locked",
...
function handleAdminCommand(interaction) {
    if (!(interaction.member.roles.includes(config.botPermsRoleId) || interaction.member.permissions.has("administrator"))) {
        interaction.createMessage({
            content: `Only members with the <@&${config.botPermsRoleId}> role can use this command.`,
            flags: Constants.MessageFlags.EPHEMERAL
        });
        return;
    }
